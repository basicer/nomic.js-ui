...
export default function Inspect() {
	...
	const theme = useMediaQuery('(prefers-color-scheme: dark)')?'chromeDark':'chromeLight';

	const nodeRenderer = ({ depth, name, data, isNonenumerable, expanded }) => {
		if ( depth === 0 )  return (<ObjectRootLabel name={name} data={data} />);
		if ( (name == "src" || name == "code") && typeof data == "string" && data.length > 0 ) {
			return (
			<span><Highlight language="javascript">
			{data}
			</Highlight></span>)
		}
	...
	return (<Paper>
	<ErrorBoundary>
	<ObjectInspector data={filtered} theme={theme} expandLevel={3} nodeRenderer={nodeRenderer} />
	{/*
    <TreeView
